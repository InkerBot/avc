...
function portLabel(name: string, t: TFunction): string {
...
export function AvcNode({ id, data, selected }: NodeProps<AvcNodeType>) {
  ...
  const label = t(`nodes.${data.type}.label`, { defaultValue: descriptor.label })
  ...
  return (
    <div
      className={`node${selected ? ' node--selected' : ''}${data.domain ? ' node--cold' : ''}`}
    ...
    >
      ...
      {targetParam && (
        <div className={`node__endpoint${target ? '' : ' node__endpoint--unset'}`}>
        ...
        <div className="node__column">
          {inputs.map((name, i) => (
            <div className="port" key={name}>
              ...
              />
              <span className="port__name">{portLabel(name, t)}</span>
